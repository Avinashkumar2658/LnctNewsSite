...
import com.lnct.news.services.TnpService;
...
public class TnpController {
	...
	@Autowired
	TnpService tnpService;

	@GetMapping("/tnp_update/{semester}")
	public ResponseEntity<List<ResponseFile>> getListFiles(@PathVariable int semester) {
		List<ResponseFile> files = tnpService.findTnpUpdate(semester).map(dbFile -> {
			String fileDownloadUri = ServletUriComponentsBuilder.fromCurrentContextPath().path("/tnp_update/semester/")
	...
	@GetMapping("/tnp_update/semester/{largeNewsId}")
	public ResponseEntity<byte[]> getFile(@PathVariable Long largeNewsId) {
		LargeNews fileDB = tnpService.getFile(largeNewsId);
